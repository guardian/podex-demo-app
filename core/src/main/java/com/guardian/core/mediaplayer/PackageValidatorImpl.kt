 ...
 * its signature, this class will print to Timber (INFO level) a message with the proper
 * xml tags to add to allow the caller.
 *
 * For more information, see res/xml/allowed_media_browser_callers.xml.
 */
class PackageValidatorImpl(context: Context, @XmlRes xmlResId: Int) : PackageValidator {
    ...
    override fun isKnownCaller(callingPackage: String, callingUid: Int): Boolean {
        ...
        if (!isCallerKnown) {
            Timber.i("$callingPackage with id $callingUid is unknown")
        }
    ...
    private fun buildCallerInfo(callingPackage: String): CallerPackageInfo? {
        ...
        return CallerPackageInfo(
            appName,
            callingPackage,
            uid,
            signature,
            activePermissions.toSet()
        )
    }
    ...
    private fun getSignature(packageInfo: PackageInfo): String? =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            if (packageInfo.signingInfo?.apkContentsSigners == null || packageInfo.signingInfo?.apkContentsSigners?.size != 1) {
                ...
                val certificate = packageInfo.signingInfo.apkContentsSigners[0].toByteArray()
    ...
    private fun parseV1Tag(parser: XmlResourceParser): KnownCallerInfo {
        ...
        val callerSignature =
            KnownSignature(signature, isRelease)
        return KnownCallerInfo(
            name,
            packageName,
            mutableSetOf(callerSignature)
        )
    }
    ...
    private fun parseV2Tag(parser: XmlResourceParser): KnownCallerInfo {
        ...
        while (eventType != XmlResourceParser.END_TAG) {
            ...
            callerSignatures += KnownSignature(
                signature,
                isRelease
            )
